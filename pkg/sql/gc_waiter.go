...
func (r *waitForGCResumer) updateDeadlines(
	ctx context.Context,
...
func (r waitForGCResumer) Resume(
	...
	timer := time.NewTimer(sqlGcInterval)

	// Ensure that the deadlines of the elements we want to drop are up to date.
	if err := r.updateDeadlines(ctx, execCfg, zoneCfgFilter, cfgFilter); err != nil {
		return err
	}
		...
		case <-timer.C:
			if err := execCfg.DB.Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
				if details.Indexes != nil {
					// Drop indexes.
					table, err := sqlbase.GetTableDescFromID(ctx, txn, details.IndexParentID)
					if err != nil {
						return err
					}
					...
					if details.DatabaseID != sqlbase.InvalidID {
						if err := cleanupDatabaseZoneConfig(ctx, txn, details.DatabaseID); err != nil {
							return err
						}
					}
				}
