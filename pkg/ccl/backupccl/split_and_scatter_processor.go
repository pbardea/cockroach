...
type splitAndScatterer interface {
	...
	splitAndScatterKey(ctx context.Context, codec keys.SQLCodec, db *kv.DB, kr *storageccl.KeyRewriter, key, endKey roachpb.Key, randomizeLeases bool) (roachpb.NodeID, error)
}
...
func (s dbSplitAndScatterer) splitAndScatterKey(
	...
	if len(nextKey) > 0 {
		newNextKey, err := rewriteBackupSpanKey(codec, kr, nextKey)
		if err != nil {
			return 0, err
		}

		log.VEventf(ctx, 1, "presplitting new key %+v", newNextKey)
		if err := db.AdminSplit(ctx, newNextKey, expirationTime); err != nil {
			return 0, errors.Wrapf(err, "splitting key %s", newNextKey)
		}
	}
...
func (ssp *splitAndScatterProcessor) Next() (rowenc.EncDatumRow, *execinfrapb.ProducerMetadata) {
	...
	log.Infof(ssp.Ctx, "num entries: %d", len(ssp.doneScatterCh))
...
func (ssp *splitAndScatterProcessor) runSplitAndScatter(
	...
	g.GoCtx(func(ctx context.Context) error {
		...
		for i, importSpanChunk := range spec.Chunks {
			log.Infof(ctx, "pbardea: num chunks %d", len(spec.Chunks))
			nextChunkIdx := i + 1
			var nextChunkKey roachpb.Key
			if nextChunkIdx < len(spec.Chunks) {
				nextChunkKey = spec.Chunks[nextChunkIdx].Entries[0].Span.Key
			}
			_, err := scatterer.splitAndScatterKey(ctx, flowCtx.Codec(), db, kr, importSpanChunk.Entries[0].Span.Key, nextChunkKey, true /* randomizeLeases */)
			if err != nil {
				return err
			}
	...
	for worker := 0; worker < splitScatterWorkers; worker++ {
		g.GoCtx(func(ctx context.Context) error {
			for importSpanChunk := range importSpanChunksCh {
				log.Infof(ctx, "pbardea num entries %d", len(importSpanChunk))
				log.Infof(ctx, "processing a chunk")
				for i, importSpan := range importSpanChunk {
					nextChunkIdx := i + 1
					var nextChunkKey roachpb.Key
					if nextChunkIdx < len(spec.Chunks) {
						nextChunkKey = importSpanChunk[nextChunkIdx].Span.Key
					}
					log.Infof(ctx, "processing a span [%s,%s)", importSpan.Span.Key, importSpan.Span.EndKey)
					destination, err := scatterer.splitAndScatterKey(ctx, flowCtx.Codec(), db, kr, importSpan.Span.Key, nextChunkKey, false /* randomizeLeases */)
					if err != nil {
						return err
					}
