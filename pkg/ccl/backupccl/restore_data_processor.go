...
func (rd *restoreDataProcessor) Start(ctx context.Context) {
	...
	go func() {
		defer close(entries)
		if err := inputReader(ctx, rd.input, entries, rd.metaCh); err != nil {
			select {
			case rd.restoreErr <- err:
			default:
			}
		}
	}()
	...
	go func() {
		...
		if err := rd.runRestoreWorkers(entries); err != nil {
			select {
			case rd.restoreErr <- err:
			default:
			}
