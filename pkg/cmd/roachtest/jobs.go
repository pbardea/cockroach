...
func jobSurvivesNodeShutdown(
	ctx context.Context, t *test, c *cluster, gatewayNode, nodeToShutdown int, query string,
) {
	...
	t.l.Printf("test has chosen gateway node %d, shutdown target node %d, and watcher node %d",
		gatewayNode, nodeToShutdown, watcherNode)
	...
	m.Go(func(ctx context.Context) error {
		...
		if err := gatewayDB.QueryRowContext(ctx, query).Scan(&jobID); err != nil {
			return errors.Wrap(err, "running the job")
		}
		t.l.Printf("started running job with ID %s on node %d", jobID, gatewayNode)
		jobIDCh <- jobID
		close(jobIDCh)
